...
```mermaid
graph TD
    A[App Layer 应用层] --> B[Command Layer 命令层]
    A --> C[Model Layer 模型层]
    A --> D[Utils Layer 工具层]
    B --> C
    D --> C
...
#### Lab1 基础功能测试（TextEditorTest.java）
...
**注**：当前测试文件 `TextEditorTest.java` 主要覆盖 Lab1 的基础功能。Lab2 新增功能（XML 编辑、拼写检查、统计模块）的自动化测试用例待补充，但所有功能已通过手动功能验证测试。

建议后续补充以下测试类：

- `XmlEditorTest.java`：XML 编辑和约束验证
- `XmlCommandTest.java`：XML 命令的 undo/redo
- `SpellCheckerTest.java`：拼写检查功能
- `SessionStatisticsTest.java`：统计模块
